...
from huobi.HuobiDMUtil import http_get_request, api_key_post
...
class HuobiDM:
    ...
    # 获取KLine
    def get_contract_kline_ex(self, symbol, period, size=0, fromtime=0, totime=0):
        """
        :param fromtime:
        :param symbol  BTC_CW, BTC_NW, BTC_CQ , ...
        :param period: 可选值：{1min, 5min, 15min, 30min, 60min, 4hour, 1day, 1week, 1mon }
    ...
    # 闪电平仓下单
    def send_lightning_close_position(self, symbol, contract_type, contract_code,
    ...
    # 获取计划委托当前委托
    def get_contract_trigger_openorders(self, symbol, contract_code=None, page_index=None, page_size=None):
        """
        @author: liaoy
        属性	        数据类型	    是否必填	说明
    ...
    # 合约计划委托下单
    def send_contract_trigger_order(self, symbol, contract_type, contract_code, trigger_type,
